<script lang="ts">
  ...
  const ComputeTooltip = (): Array<Array<string | string[]>> => {
    const total = _.sum(_.map(accountBudget.expenses, (p) => p.amount))
    return _.chain(accountBudget.expenses)
      .groupBy((p) => p.payee)
      .map((group, payee) => {
        return {
          payee: payee,
  ...
  const tooltipContent = tooltip(ComputeTooltip(), {
    total: formatCurrency(_.sum(_.map(accountBudget.expenses, (p) => p.amount)))
  });
</script>
